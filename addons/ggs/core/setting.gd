@tool
extends Resource
abstract class_name GGSSetting
## A game setting is a resource that contains the necessary information for a specific game
## setting such as its default value or its script.

## Default value of the setting.
var default: Variant = false: set = _set_default

## The section name used to save and load the value from the save file.
var section: String = ""

## The key name used to save and load the value from the save file.
var key: String = "" : set = _set_key

## The value type this setting uses.
var type: Variant.Type = TYPE_BOOL

## Property hint of [member default]. Use it to customize how [member default] is exported.
var hint: PropertyHint = PROPERTY_HINT_NONE

## Property hint string of [member default]. Use it alongside [member hint] to customize how
## [member default] is exported.
var hint_string: String = ""


func _get_property_list() -> Array:
	var properties: Array
	properties.append_array([
		{
			"name": "default",
			"type": type,
			"hint": hint,
			"hint_string": hint_string,
		},
		{
			"name": "section",
			"type": TYPE_STRING,
		},
		{
			"name": "key",
			"type": TYPE_STRING,
		},
	])
	return properties


func _set_default(value: Variant) -> void:
	default = value

	# if Engine.is_editor_hint() and not key.is_empty():
	# 	GGS.set_value(self, value)


func _set_key(value: String) -> void:
	key = value
	resource_name = value
	emit_changed()


## This method is called when a setting needs to be applied. In other words, it should
## contain the setting logic. Extra arguments can be passed to the method if needed.
abstract func apply(value, ...extra_args: Array) -> void
