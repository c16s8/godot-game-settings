...
static func get_all_settings() -> Array[GGSSetting]:
    ...
    var settings: PackedStringArray = _get_dir_settings(plugin_settings.settings_directory)
...
static func _show_error_empty_key(setting: GGSSetting) -> void:
    printerr("GGS::Save Setting - Failed to save setting '%s'. Key is empty."%[setting.resource_path.get_file()])